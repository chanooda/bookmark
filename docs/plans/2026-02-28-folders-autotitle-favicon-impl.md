...
import type { CreateFolderDto, UpdateFolderDto } from '@bookmark/types';
...
```ts
import type { CreateFolderDto, Folder, UpdateFolderDto } from '@bookmark/types';
...
Note: check what `@/shared/api` exports. If `apiClient` isn't exported from there, import from `@bookmark/api-client` directly.

**Step 2: Create `apps/web/src/shared/ui/FolderSelect.tsx`**

```tsx
import type { Folder } from '@bookmark/types';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@bookmark/ui/components/select';
